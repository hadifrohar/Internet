...
(function () {
    ...
    let Utils = (function () {
         ...
         */
        const getGithubProfile = (data) => 'https://github.com/'+data.login;
    ...
    function save_user()
    {
        ...
        fetch('./users/save', Utils.postUserParam(username)).then(function(response) {
            if(response.status !== 200) {
                Utils.alertBox.show('Username already saved!', true);
                return;
            }

            response.json().then(function(data) {
                Utils.alertBox.show(data.msg, false);
    ...
    function delete_user() {
        ...
        fetch('./users/delete', Utils.postUserParam(username)).then(function(response) {
            if(response.status !== 200) {
                Utils.alertBox.show('No such username!', true);
                return;
            }
     ...
     */
    function init_saved() {
        fetch('./users/getSaved', {method: 'GET', headers: {'Content-Type': 'application/json'}}).then(
            function(response) {
                if(response.status !== 200) {
                    Utils.alertBox('Error ' + response.status + ': ' + response.statusText, true);
                    return;
                }
                response.json().then(function(data) {
                    Utils.getSavedBox().innerHTML = '';
                    Utils.getSavedBox().appendChild(new HTMLList('Saved users', data.users, Utils.getLogin,
                        Utils.getGithubProfile).getList());
                })
            }
        )
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('user_input').addEventListener('input', Utils.alertBox.hide);
    }, false);
